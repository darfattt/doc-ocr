...
public class AttachmentServiceImpl implements AttachmentService {
	...
	AttachmentServiceImpl(AttachmentGroupRepository attachmentGroupRepository,
                          AttachmentRepository attachmentRepository, FileManager fileManager) {
	...
	public <T> AttachmentGroup createAttachmentGroup(Class<T> object, String attachmentGroupParentId,
		...
		synchronized (this) {
			...
			if (attachmentGroupParentId != null) {
				attachmentGroup.setAttachmentGroupParent(
						attachmentGroupRepository.findById(attachmentGroupParentId).orElse(null));
			}
	...
	public Boolean removeGroup(AttachmentGroup attachmentGroupRequest) {
		AttachmentGroup attachmentGroup = attachmentGroupRepository.findById(attachmentGroupRequest.getId())
		...
		List<Attachment> attachments = attachmentRepository
				.findAllByAttachmentGroupIdOrderByCreatedDateDesc(attachmentGroup.getId());
		for (Attachment attachment : attachments) {
	...
	public OutputContentFile downloadAttachment(String id) {
        ...
        try {
            return fileManager.getFile(attachment.getAttachmentGroup().getBasePath(), attachment.getId(),
                    attachment.getName());
        } catch (Exception e) {
	...
	public String generateUrl(Attachment attachment) {
		...
		url.append(staticResourceHost).append("/")
	...
	public List<Attachment> saveAttachment(AttachmentRequest attachmentRequest) {
		...
		if (attachmentRequest.getAttachmentGroupId() == null) {
			attachmentGroupResult = this.createAttachmentGroup(attachmentRequest.getClassName(),
					...
					attachmentRequest.getName(), attachmentRequest.getBasePath());
		}
		else {
		...
		List<Attachment> inputAttachments = attachmentRequest.getAttachments().stream()
				.map(attachment -> ObjectMapperUtil.MAPPER.convertValue(attachment, Attachment.class)).collect(Collectors.toList());
		List<Attachment> attachmentResult = this.saveAttachment(attachmentGroupResult, new HashSet<>(inputAttachments),
				null);
		return attachmentResult;
	}
	...
	private void removeFile(Attachment attachment) {
        try {
            fileManager.deleteFile(attachment.getAttachmentGroup().getBasePath(),
	...
	private String buildPathWithYear(String path) {
		...
		return new StringBuilder().append(year).append(File.separator).append(path).append(File.separator).toString();
	}
